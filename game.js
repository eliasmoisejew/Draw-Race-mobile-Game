...
// Tutorial track — normalized (0–1) relative to canvas size.
// Starts top-left, sweeps right, curves down the right side,
// S-curve through the middle, exits bottom-right.
const TUTORIAL_PATH_NORM = [
  { x: 0.14, y: 0.18 },
  { x: 0.28, y: 0.14 },
  { x: 0.44, y: 0.13 },
  { x: 0.60, y: 0.15 },
...
class DrawRaceGame {
  ...
  resize() {
    ...
    if (this.tutorialMode) this.tutorialPts = this.buildTutorialPts();
  }
  ...
  bindInput() {
    ...
    const move = (e) => {
      ...
      if (Math.hypot(p.x - last.x, p.y - last.y) >= RESAMPLE_DIST) this.rawPts.push(p);
    };
  ...
  buildTutorialPts() {
    ...
    const raw = TUTORIAL_PATH_NORM.map(p => ({ x: p.x * W, y: p.y * H }));
    return this.catmullRom(this.resample(raw, RESAMPLE_DIST), CATMULL_STEPS);
  }
  ...
  tick(dt) {
    ...
    // Animate tutorial guide arrows
    if (this.tutorialMode) {
      this.arrowOffset = (this.arrowOffset + 0.28 * (dt / 16)) % 42;
      this.pulseT      = (this.pulseT + dt * 0.002) % (Math.PI * 2);
    }
  ...
  moveCar(dt) {
    ...
    const i0 = Math.floor(car.idx), i1 = Math.min(i0+1, pts.length-1);
    const f  = car.idx - i0;
  ...
  spawnParticle(car) {
    const ba = car.angle + Math.PI, sp = car.speed * 0.6 + 0.3;
    this.particles.push({
  ...
  finishRace() {
    ...
    this.car.x = last.x; this.car.y = last.y;
    this.elapsed = performance.now() - this.startMs;

    const isNew = !this.bestMs || this.elapsed < this.bestMs;
    if (isNew) { this.bestMs = this.elapsed; localStorage.setItem('dr_best', this.bestMs); }
  ...
  render() {
    ...
    // Ghost tutorial guide — drawn first (behind everything)
    if (this.tutorialMode && this.tutorialPts.length > 1) {
      this.drawGhostTrack();
    }
  ...
  // ── Tutorial ghost track ───────────────────────────────────────────────────

  drawGhostTrack() {
    const { ctx, tutorialPts: pts } = this;
    // Pulsing alpha driven by pulseT
    ...
    ctx.lineWidth   = TRACK_WIDTH + 26;
    this.tracePath(ctx, pts); ctx.stroke();

    // Road fill
    ctx.globalAlpha = 1;
    ctx.strokeStyle = 'rgba(0,25,45,0.96)';
    ctx.lineWidth   = TRACK_WIDTH;
    this.tracePath(ctx, pts); ctx.stroke();

    // Cyan border lines
    ctx.strokeStyle = `rgba(0,200,255,${0.40 * pulse})`;
    ctx.lineWidth   = TRACK_WIDTH;
    this.tracePath(ctx, pts); ctx.stroke();

    // Refill centre
    ctx.strokeStyle = 'rgba(0,12,28,0.98)';
    ctx.lineWidth   = TRACK_WIDTH - 7;
    this.tracePath(ctx, pts); ctx.stroke();

    // Dashed centre line
    ctx.strokeStyle = `rgba(0,200,255,${0.30 * pulse})`;
    ctx.lineWidth   = 1.5;
    ...
    // "TUTORIAL GUIDE" label near start
    this.drawGuideLabel(pts[0], pts[Math.min(8, pts.length-1)]);

    // Start / finish ghost markers
    this.drawGhostMarker(pts[0],     'rgba(0,255,136,0.65)', 'S');
    this.drawGhostMarker(pts.at(-1), 'rgba(255,51,85,0.65)',  'F');

    // Animated direction arrows
    this.drawGhostArrows(pts, pulse);
  }

  drawGuideLabel(start, tip) {
    const ctx   = this.ctx;
    const angle = Math.atan2(tip.y - start.y, tip.x - start.x);
    // Place label slightly behind the start point
    const lx = start.x - Math.cos(angle) * 36;
    const ly = start.y - Math.sin(angle) * 36;
    ...
    ctx.fillText('TUTORIAL GUIDE', lx, ly - 28);
    ctx.restore();
  }

  drawGhostMarker(pt, color, label) {
    const ctx = this.ctx;
    ctx.save();
  ...
  drawGhostArrows(pts, pulse) {
    const ctx     = this.ctx;
    const spacing = 42;
    const offset  = Math.floor(this.arrowOffset) % spacing;

    ctx.save();
    for (let i = offset + 18; i < pts.length - 8; i += spacing) {
      const p     = pts[i];
      const ahead = pts[Math.min(i + 5, pts.length - 1)];
  ...
  drawMarkers(pts) {
    this.drawMarker(pts[0],     '#00ff88', 'S');
    this.drawMarker(pts.at(-1), '#ff3355', 'F');
  }
  ...
  resetToIdle() {
    ...
    // Rebuild tutorial points for current screen size
    if (this.tutorialMode) this.tutorialPts = this.buildTutorialPts();
    ...
    // Hint text differs between normal and tutorial
    const h3 = document.querySelector('#hint h3');
    const p  = document.querySelector('#hint p');
    if (this.tutorialMode) {
      h3.textContent = 'Trace the Track!';
      p.textContent  = 'Draw along the glowing blue guide';
    } else {
      h3.textContent = 'Draw Your Track';
      p.textContent  = 'Hold and drag to draw your race path';
    }
  }
}
